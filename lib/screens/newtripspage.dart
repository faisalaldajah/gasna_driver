...
import 'package:gasna_driver/datamodels/tripdetails.dart';
import 'package:gasna_driver/helpers/helpermethods.dart';
import 'package:gasna_driver/helpers/mapkithelper.dart';
import 'package:gasna_driver/widgets/CollectPaymentDialog.dart';
...
class _NewTripPageState extends State<NewTripPage> {
  ...
  String buttonTitle = 'وصول الى الوجهة';
                      ...
                      Row(
                        children: <Widget>[
                          ...
                          Expanded(
                            child: Container(
                              child: Text(
                                '${numberOfGas.toString()} من الاجرار الغاز',
                                style: TextStyle(fontSize: 18),
                                overflow: TextOverflow.ellipsis,
                              ),
                            ),
  ...
  void endTrip() async {
    ...
    double fares = HelperMethods.estimateFares();
    ...
    showDialog(
      ...
      builder: (BuildContext context) => CollectPayment(
        ...
        fares: fares,
        finishCode: widget.tripDetails.finishCode,
      ),
    );
    topUpEarnings(fares);
  }
    ...
    earningsRef.once().then((DataSnapshot snapshot) {
      if (snapshot.value != null) {
        ...
        double adjustedEarnings =
            (fares.toDouble() - (0.25 * numberOfGas)) + oldEarnings;

        earningsRef.set(adjustedEarnings.toStringAsFixed(2));
      } else {
        double adjustedEarnings = (fares.toDouble() - (0.25 * numberOfGas));
        earningsRef.set(adjustedEarnings.toString());
      }
    });
  }
}
